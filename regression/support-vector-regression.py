...
##################################################
##################################################
plt.        scatter(sc_X.       inverse_transform(X.   reshape(len(X),1)), 
                    sc_y.       inverse_transform(y.   reshape(len(y),1)), 
                    color='red') # original data points

plt.        plot(sc_X.       inverse_transform(X.   reshape(len(X),1)), 
                 sc_y.       inverse_transform(regressor.  predict(X).   reshape(len(X),1)),
                 color='blue') # SVR model predictions
...
##################################################
##################################################
X_grid      =   np.        arange(sc_X. inverse_transform(X. reshape(len(X),1)). min(),
                                  sc_X. inverse_transform(X. reshape(len(X),1)).  max(), 
                                  0.01) # create a grid with a step of 0.01
...
plt.        plot(X_grid, 
                 sc_y. 
                 inverse_transform(regressor.  
                                        predict(sc_X.  
                                                  transform(X_grid)). 
                                                                 reshape(len(X_grid),1)),
                 color = 'blue') # SVR model predictions
